...
def get_device_names():
    """Use 2nd nornir instance to get list of devices"""

    # Return list of ios devices
    device_names = nr.inventory.hosts.keys()
    return device_names
...
class TestDevices:
    ...
    def test_bgp_neighbours(self, pytestnr, device_name):
        ...
        for neighbour in bgp_neighbours:
            ...
            if "eos" in device_name:
                if "Estab" in neighbour['state']:
                    actual_neighbours += 1
            # NXOS or IOS devices
            else: 
                if (
    ...
    def test_interface_status(self, pytestnr, device_name):
        ...
        for interface in interfaces:
            if 'up' not in interface['status']:
                if 'eos' in device_name:
                    down_interfaces.append(interface['interface'])
                else:
                    down_interfaces.append(interface.get('intf'))
    ...
    def test_version(self, pytestnr, device_name):
            ...
            elif "nxos" in device_name:
                actual_version = version[0]['os']
            elif "eos" in device_name:
                actual_version = version[0]['image']
